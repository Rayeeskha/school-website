...
class EventReposity extends BaseRepository
{
	use UploadFile;
    ...
    public function index(){
        $data = Event::select('*',\DB::raw('@rownum  := @rownum  + 1 AS DT_RowIndex'))->orderBy('id', 'DESC');
        return Datatables::of($data)->addIndexColumn()
            ->addColumn('action', function($row){
                $btn = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" class="edit btn btn-info btn-md editEvent"><span class=" bx bx-edit text-white"></span></a>'; 
                ...
                return '<a download href="'.asset($row->photo).'" /> Download</a>';
            ...
            })->rawColumns(['status','action','attachment'])->make(true);
    }
    ...
    public function store($data){
        ...
        if (!empty($data['attachment'])) {
            $data['attachment'] =  $this->uploadImage($data['attachment'], 'uploads/attachment/events', 'events', ['id' => $id ], 'attachment');
        }
